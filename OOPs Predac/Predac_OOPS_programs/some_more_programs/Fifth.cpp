/*friend function is a function which can access private member of a class in which it is declared as "friend".
*/
...
public:
	...
	~Student()
	{
		cout<<"inside Student destructor\t"<<name<<endl;
	}
	static int getCenter_id()
	{
		return center_id;
	}
};
int Student::center_id=1;
int main()
{
	Student s1;
	...
	s4->setName("sachin");
	s4->setAge(23);
	s4->setQualification("be");
	cout<<s4->getRollno()<<"\t"<<s4->getName()<<"\t"<<s4->getAge()<<"\t"<<s4->getQualification()<<endl;
	delete s4;

	cout<<Student::getCenter_id()<<endl;
	return 0;
}